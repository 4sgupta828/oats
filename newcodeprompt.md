...
**❌ DON'T:**
- Read entire file into context
- Copy truncated data to new files

**✅ DO:**
- Use streaming tools: `grep`, `jq`, `awk`, `head`, `tail`
- Write Python script to process line-by-line
...
**Default Behavior**: Honor .gitignore patterns to avoid noise
...
# Or filter with grep
grep -r "pattern" . --exclude-dir={node_modules,venv,.git,dist}
```
...
**Intentional Override**: If goal explicitly requires examining ignored files (logs, artifacts), state in reasoning: *"Goal requires examining log files which are typically ignored - intentionally including them."*
